...
export const scrollAnimationDefaultProps = {
  ...
  content: 'Engage your users with beautiful, scroll-triggered animations that bring your content to life. Add high-quality imagery to make it stand out.',
  showButton: true,
...
export default function ScrollAnimationComponent(incomingProps) {
  ...
  const getVariants = () => {
    ...
    const visible = { opacity: 1, x: 0, y: 0, scale: 1, rotate: 0, filter: 'blur(0px)' };

    switch (animationType) {
      case 'fade-up': return { hidden: { ...baseHidden, y: safeDistance }, visible };
      case 'fade-down': return { hidden: { ...baseHidden, y: -safeDistance }, visible };
      case 'fade-in': return { hidden: baseHidden, visible };
      case 'zoom-in': return { hidden: { ...baseHidden, scale: safeScale }, visible };
      case 'zoom-out': return { hidden: { ...baseHidden, scale: Math.max(1, safeScale) }, visible };
      case 'slide-left': return { hidden: { ...baseHidden, x: safeDistance }, visible };
      case 'slide-right': return { hidden: { ...baseHidden, x: -safeDistance }, visible };
      case 'rotate-in': return { hidden: { ...baseHidden, rotate: safeRotate, scale: safeScale }, visible };
      case 'blur-in': return { hidden: { ...baseHidden, y: safeDistance / 2 }, visible };
      default: return { hidden: { ...baseHidden, y: safeDistance }, visible };
    }
  };

  // To prevent constant re-triggering while editing, we force visibility in edit mode
  const animateProps = (isEditable && !isPreviewing)
  ...
  const effectivePaddingY = Number.isFinite(Number(paddingY)) ? Number(paddingY) : padding;
  const effectivePaddingX = Number.isFinite(Number(paddingX)) ? Number(paddingX) : 20;
  ...
  const textAlignmentClass = getTextAlignmentClasses(isStacked ? 'center' : alignment);
  ...
  const contentBlock = (
    <div
      ...
      style={{ color: textColor, maxWidth: isStacked || isFullWidth ? '860px' : contentMaxWidth }}
    >
      {showEyebrow && eyebrow && (
        <p
      ...
      {(showButton && buttonText) || (showSecondaryButton && secondaryButtonText) ? (
        <div className={`mt-8 flex flex-wrap gap-3 ${alignment === 'center' || isStacked ? 'justify-center' : alignment === 'right' ? 'justify-end' : 'justify-start'}`}>
  ...
  const imageBlock = showImage && imageUrl ? (
    <div className="relative min-w-0 flex-1 overflow-hidden" style={{ borderRadius: `${imageRadius}px`, backgroundColor: imageBackgroundColor }}>
      <img
        src={imageUrl}
        alt={imageAlt}
      ...
      {showImageOverlay && (
        <div
          ...
          style={{ backgroundColor: imageOverlayColor, borderRadius: `${imageRadius}px` }}
        />
      )}
    </div>
  ) : null;
  ...
  return (
    ...
    >
      ...
      >
        {orderedChildren.map((child, index) => child ? (
